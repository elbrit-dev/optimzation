...
export default function DataProviderNew({
  ...
  const checkIndexedDBAndLoadData = useCallback(async (queryId, queryDoc, monthRangeValue) => {
    ...
    // Skip index checks if dataSource is null (offline mode / drawer tables)
    if (!queryId) {
      // No queryId (dataSource is null), skip index checks and go directly to API/offline data
      cacheLoadInProgressRef.current = null;
      setLoadingFromCache(false);
      if (runQueryRef.current) {
        await runQueryRef.current(queryId, true);
  ...
  const rawTableData = useMemo(() => {
    ...
    else if (dataSource && processedData && selectedQueryKey) {
      result = getDataValue(processedData, selectedQueryKey) || [];
    }
    else {
      result = null;
    }
  ...
  // Declare mainTableEditingDataRef early so it can be used in tableData
  // This ref will hold the editing data (user edits)
  const mainTableEditingDataRefEarly = useRef([]);
  // State to trigger tableData recalculation when editing data changes
  // Declared early so it can be used in tableData useMemo
  const [tableDataUpdateTrigger, setTableDataUpdateTrigger] = useState(0);

  // Helper function to generate unique immutable editing key
  // Declared early so it can be used in tableData
  const generateEditingKey = useCallback((index) => {
    return `__editingKey__${Date.now()}_${Math.random().toString(36).substr(2, 9)}_${index}`;
  }, []);

  // Helper function to add editing keys to rows
  // Declared early so it can be used in tableData
  const addEditingKeysToRows = useCallback((data) => {
    if (!isArray(data)) return data;
    return data.map((row, index) => {
      if (!row || typeof row !== 'object') return row;
      // Preserve existing __editingKey__ if present, otherwise generate new one
  ...
  // Merge preFilteredData with mainTableEditingData (user edits)
  // This happens after pre-filtering but before search/filter/sort
  // NOTE: We use mainTableEditingDataRefEarly.current to check for edits
  // The state mainTableEditingData will sync with this ref later
  const tableData = useMemo(() => {
    // Check if mainTableEditingDataRefEarly has data (user edits)
    const editingData = mainTableEditingDataRefEarly.current;
    if (editingData && isArray(editingData) && !isEmpty(editingData)) {
      return editingData;
    }
    // Fall back to preFilteredData if no edits
    // CRITICAL: Ensure preFilteredData has __editingKey__ so drawer can find parent row
    const dataWithKeys = preFilteredData && isArray(preFilteredData) && !isEmpty(preFilteredData)
      ? addEditingKeysToRows(preFilteredData)
      : preFilteredData;
    return dataWithKeys;
  }, [preFilteredData, tableDataUpdateTrigger, addEditingKeysToRows]); // CRITICAL: Depend on trigger state so tableData recalculates when editing data changes
  ...
  // Reset search, sort, and pre-filters when data source changes
  useEffect(() => {
    setSearchTerm('');
    setSortConfig(null);
    setPreFilterValues({});
  ...
  // Apply search filter after merging edits (only when clientSave is true)
  ...
  const searchedData = useMemo(() => {
    ...
    // Rebuild search index for tableData (since it may have different rows than preFilteredData)
    // For now, use simple search on tableData
    const searchFieldsObj = queryDoc.searchFields;
  ...
  const extractJsonNestedTablesFromData = useCallback((data, maxDepth = 10) => {
    ...
    const result = extractJsonNestedTablesRecursive(data, 0, maxDepth);
    return result;
  }, []);
  ...
  // Paginated data computation - use editingData instead of sortedData
  // Use state for reactivity but sync with ref
  const paginatedData = useMemo(() => {
    // CRITICAL: Use sortedData (which has __nestedTables__ extracted) instead of mainTableEditingData
    // sortedData comes from filteredDataWithNestedTables which has the nested tables properly extracted
    const dataToPaginate = sortedData;
    if (!isArray(dataToPaginate) || isEmpty(dataToPaginate)) {
      return [];
    }
    const sliced = dataToPaginate.slice(tablePagination.first, tablePagination.first + tablePagination.rows);
    // Keep __nestedTables__ in paginatedData for rendering (needed for expander button)
    // But exclude it from columns list (see columns computation above)
    return sliced;
  ...
  }, [nestedTableTabId, parentNestedTableEditingDataRef]);
  ...
  const handleDrawerSave = useCallback(() => {
    ...
    const firstRowEditing = drawerEditingData[0];
    const firstRowOriginal = trackingData.originalData[0];
  ...
  const exportToXLSX = useCallback((selectedLevelIndices) => {
    ...
    if (effectiveGroupFields.length > 0 && !isEmpty(groupedData)) {
      ...
      selectedLevelIndices.forEach((levelIndex) => {
        ...
        const cleanRows = rows.map((row) => {
          const clean = {};
          allColumns.forEach((col) => { clean[col] = getDataValue(row, col); });
          return clean;
        });
        const exportData = cleanRows.map((row) => formatRowForExport(row, allColumns));
    ...
    // Write file
    XLSX.writeFile(wb, filename);
  }, [enableBreakdown, reportData, columnGroupBy, sortedData, groupedData, effectiveGroupFields, hasPercentageColumns, percentageColumns, isPercentageColumn, getPercentageColumnValue, formatHeaderName, isTruthyBoolean, formatDateValue, getColumnTypeFlags, derivedColumns, collectSummaryRowsAtLevel, sanitizeSheetName]);
  ...
  return (
    <>
              ...
              {hasDrawerTabs ? (
                ...
                >
                  {drawerTabs.map((tab, index) => {
                    ...
                    let tabDataSource;
                    if (isJsonTable) {
                      // Use editing buffer for JSON table tabs
                      // Always prefer editing buffer over tabData to preserve user edits
      ...
      {!parentColumnName && effectiveGroupFields?.length > 0 && (
        ...
        >
          <p className="mb-3 text-gray-600">Select which group levels to export. Each selected level becomes a separate sheet in the Excel file.</p>
          <MultiSelect
            value={exportGroupSelectorSelectedLevels}
            onChange={(e) => setExportGroupSelectorSelectedLevels(e.value ?? [])}
